...
/// Generated twin of the parent's `split_block_body`: identical grammar and
/// AST split. The error is stubbed to a bare span-only error, which is enough
/// to fail the fast path (triggering the rich re-run) when a `;` is missing.
fn split_block_body<'tokens, 'src>(
    elements: Vec<(Statement, Option<Token<'src>>)>,
    emit: &mut chumsky::input::Emitter<Simple<'tokens, Token<'src>, Span>>,
    ...
    for (index, (statement, semi)) in elements.into_iter().enumerate() {
        ...
        match statement {
            ...
            statement => {
                emit.emit(Simple::new(None, *statement.span()));
                statements.push(statement);
            }
        }
    }
        ...
        I: ValueInput<'tokens, Token = Token<'src>, Span = Span>,
    {
        recursive(|expr| {
            let block = {
                ...
                let element =
                    Statement::bare_parser(expr.clone()).then(just(Token::Semi).or_not());
